...
public class CameraClient2 implements Runnable {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Thread clientThread = new Thread(new CameraClient2());
    ...
    RegistryListener createRegistryListener(final UpnpService upnpService) {
        return new DefaultRegistryListener() {
            ...
            public void remoteDeviceAdded(Registry registry, RemoteDevice device) {
                ...
                Service setFlash;
                if ((setFlash = device.findService(serviceId)) != null) {
                    System.out.println("Service discovered: " + setFlash);
                    Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(new Runnable() {
                        @Override
                        public void run() {
                            executeAction(upnpService, setFlash);
                        }
                    }, INITIAL_DELAY, VIBRATING_PERIOD, TimeUnit.SECONDS);
                    upnpService.getControlPoint().execute(createSubscriptionCallBack(upnpService, setFlash));
                }
            }
            ...
            public void remoteDeviceRemoved(Registry registry, RemoteDevice device) {
                Service setFlash;
                if ((setFlash = device.findService(serviceId)) != null) {
                    System.out.println("Service disappeared: " + setFlash);
                }
            }
    ...
    void executeAction(UpnpService upnpService, Service setFlashService) {

        ActionInvocation getTargetInvocation = new GetTargetActionInvocation(setFlashService);
        ...
        upnpService.getControlPoint().execute(
                new ActionCallback(getTargetInvocation) {
                    ...
                    public void success(ActionInvocation invocation) {
                        ...
                        ActionInvocation setTargetInvocation = new SetTargetActionInvocation(setFlashService, !((boolean) invocation.getOutput()[0].getValue()));
                        upnpService.getControlPoint().execute(
...
//        new ActionCallback.Default(getTargetInvocation, upnpService.getControlPoint()).run();
    ...
    void executeGetAction(UpnpService upnpService, Service setFlashService) {
